...
def listar_encuestas_para_alumno(db: Session, alumno_id: int):
    ...
    for encuesta in encuestas:
        ...
        # Obtener un docente de docentes_asociados (si existe alguno)
        if asignatura.docentes_asociados and len(asignatura.docentes_asociados) > 0:
            docente_asoc = asignatura.docentes_asociados[0].docente
            docente_nombre = f"{docente_asoc.nombre} {docente_asoc.apellido}"
        else:
            docente_nombre = "No asignado"
