...
def listar_encuestas_para_alumno(db: Session, alumno_id: int) -> List[schemas.EncuestaAlumnoInfo]:
    ...
    stmt = (
        select(Encuesta)
        .join(Encuesta.asignatura)  # Une encuesta con asignatura
        .where(
            Encuesta.activa == True,
            Encuesta.estado == EstadoEncuesta.abierta,
            Encuesta.fecha_inicio <= ahora,
        ...
        .options(
            selectinload(Encuesta.asignatura).selectinload(Asignatura.docentes_asociados)  # ✅ docente correcto
        )
    )
    ...
    for encuesta in encuestas:
        ...
        # ✅ Obtenemos docente desde docentes_asociados
        docente_nombre = "No asignado"
        if asignatura.docentes_asociados:
            docente = asignatura.docentes_asociados[0].docente
