...
@router.get("/docente/{docente_id}/pendientes")
def informes_pendientes_docente(docente_id: int, db: Session = Depends(get_db)):
    informes = (
        db.query(InformeCatedraFinalizado)
        .filter(
            InformeCatedraFinalizado.docente_id == docente_id,
            InformeCatedraFinalizado.finalizado == False
        )
        .all()
    )
    return informes