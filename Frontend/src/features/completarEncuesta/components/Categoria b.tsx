...
interface Props {
  categoria: Categoria;
  RespuestaOn: (pregunta_id: number, opcion_id: number | null,texto?:string) => void;
  TotalPreguntasOn?: (id: number, cantidad: number) => void; 
}

export default function PreguntasCategoria({ categoria, RespuestaOn, TotalPreguntasOn }: Props) {
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/categorias/${categoria.id}/preguntas`)
      ...
      .then((data: Pregunta[]) => {
        ...
        TotalPreguntasOn?.(categoria.id, data.length);
      })
      .catch((err) =>
        console.error("Error al obtener las preguntas de la categoría:", err)
      );
  }, [categoria.id]);
...
const cargarOpciones = (preguntaId: number) => {
  ...
  const seleccionarOpcion = (preguntaId: number, opcionId: number) => {
    setRespuestas((prev) => {
      ...
      RespuestaOn(preguntaId, opcionId, nuevaRespuesta.texto);
      return updated;
    });
  ...
  const actualizarRespuestaTexto = (preguntaId: number, texto: string) => {
    setRespuestas((prev) => {
      ...
      const updated = { ...prev, [preguntaId]: nuevaRespuesta };
      RespuestaOn(preguntaId, nuevaRespuesta.opcion_id, texto);
      return updated;
    });
  };
    ...
    <div>
      {preguntas.length === 0 ? (
        <div className="alert alert-warning">
          No existen preguntas habilitadas para esta categoría.
        </div>
      ) : (
        preguntas.map((p, i) => (
          <ItemPregunta
